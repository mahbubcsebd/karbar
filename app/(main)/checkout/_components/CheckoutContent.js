...
const CheckoutContent = () => {
  ...
  const [isFreeDeliveryApplicable, setIsFreeDeliveryApplicable] =
    useState(false);
  ...
  // *** UPDATED LOGIC ***
  // Check for the free delivery condition based on your requirement.
  useEffect(() => {
    if (cartItems && cartItems.length > 0) {
      // Check if there is AT LEAST ONE item that requires paid delivery.
      // An item requires paid delivery if its `free_delivery` property is NOT `true`.
      const requiresPaidDelivery = cartItems.some(
        (item) => item.free_delivery !== true
      );

      // If any product requires paid delivery, then free delivery is NOT applicable.
      // Otherwise, it is applicable.
      setIsFreeDeliveryApplicable(!requiresPaidDelivery);
    } else {
      // If cart is empty, reset the free delivery flag.
      setIsFreeDeliveryApplicable(false);
    }
  }, [cartItems]);

  // Update shipping cost based on free delivery status or location selection
  useEffect(() => {
    if (isFreeDeliveryApplicable) {
      setShippingCost(0);
    } else {
      // If not eligible for free delivery, use standard rates from settings
      if (selectedValue === 'inside_dhaka') {
        setShippingCost(insideDhakaDC);
      } else if (selectedValue === 'outside_dhaka') {
        setShippingCost(outsideDhakaDC);
      } else if (selectedValue === 'dhaka_sub_area') {
                  ...
                  <div className="grid gap-[18px] lg:gap-6">
                    ...
                    <div className="delivary-area">
                      ...
                      <select
                        ...
                        // *** UPDATED: removed 'disabled' attribute to make it always workable ***
                      >
